...
exports.score_table = (req, res) => {
  const sql =
    "SELECT username,(game1+game2+game3) AS scores FROM scores ORDER BY (game1+game2+game3) DESC";
...
exports.update_note = (req, res) => {
  ...
  db.query(sql, [req.session.user.username, dateFormat], (err, result) => {
    ...
    if (result.length !== 0) {
      ...
      db.query(
        sql1,
        [noteDetails.notes, req.session.user.username, dateFormat],
        (err, result1) => {
          if (err) throw err;
        }
      );
...
exports.delete_note = (req, res) => {
  ...
  db.query(sql, [req.session.user.username, dateFormat], (err, result) => {
    ...
    if (result.length === 0) {
      ...
      db.query(
        sql2,
        [req.session.user.username, dateFormat],
        (err, result1) => {
          if (err) throw err;
          console.log(result1);
...
exports.update_task = (req, res) => {
  ...
  db.query(sql, [req.session.user.username, dateFormat], (err, result) => {
    ...
    if (result.length !== 0) {
      const sql1 =
        "UPDATE planners SET morning=?,afternoon=?,evening=? WHERE username=? AND date=?";
      db.query(
        sql1,
        [
          taskDetails.morning,
          taskDetails.afternoon,
          taskDetails.evening,
